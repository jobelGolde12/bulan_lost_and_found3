...
let getTotalLost = ref([]);
let searchQuery = ref('');

watch(
  () => props.totalLosts,
  (newItem) => {
    getTotalLost.value = newItem || [];
  },
  { immediate: true }
);
...
const filteredReports = computed(() => {
  ...
  return getTotalLost.value.filter((item) => {
    ...
    <div class="card border-0">
      <div class="card-body">
        <div class="header-container d-flex mb-3 container-fluid justify-content-between">
          <h5 class="card-title">All <span class="text-danger">Lost</span> Reports</h5>
          ...
          <table class="table">
            ...
            <tbody>
              ...
              <tr v-if="filteredReports.length === 0">
                <td colspan="3" class="text-center text-muted">No Lost Item Found.</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="mt-3 text-muted small">
          Showing {{ filteredReports.length }} items that are lost.
        </div>
      </div>
    </div>
  </div>
