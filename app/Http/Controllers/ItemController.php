       ...
       public function checkProfanity($words)
{
    ...
    foreach ($urls as $url) {
        try {
            $response = Http::timeout(10) // 10s timeout to avoid hanging
                         ->retry(3, 100) // Retry 3 times if it fails (100ms delay)
                         ->withOptions([
                             'verify' => true, // Ensure secure HTTPS certificate check
                         ])
                         ->get($url); // Perform the GET request

            // If response is OK (status 200)
            if ($response->successful()) {
                $data = $response->json(); // Convert JSON into PHP array

                // Loop through each profanity entry in the JSON
                foreach ($data as $item) {
                    // Add the 'word' field from $data to $allProfanityWords[]
                    if (isset($item['word'])) {
                        $allProfanityWords[] = $item['word'];
                    }
                }
            } else {
                // Log if the API returned an error status
                Log::warning("Failed to fetch from {$url}, status: " . $response->status());
            }
        } catch (\Exception $e) {
            // Log any connection or processing error, but continue with next URL
            Log::error("Error fetching from {$url}: " . $e->getMessage());
            continue; // Keep processing other URLs even if one fails
        }
    }

    // If both API sources failed, load profanity words from a local cache
    if (empty($allProfanityWords)) {
        $allProfanityWords = $this->getCachedProfanityWords();
    }

    // Array to store detected profanity words found in the input
    $found = [];

    // Compare each bad word against input text
    foreach ($allProfanityWords as $badWord) {
        // stripos() finds word ignoring case; !== false means it exists in the text
        if (stripos($words, $badWord) !== false) {
            $found[] = $badWord; // Add found profanity word to results
        }
    }

    // Log the input and detected words for debugging/auditing
    Log::info([
        'Checking profanity in input:' => $words,
        'Found:' => $found,
    ]);

    return $found; // Return all matched profanity words
}
